...
class _Managed:
    ...
    def colorForCanvas(self, _canvas):
        return adapter.QColor(*(round(value * 255) for value in self.values[:3]))

    @classmethod
    def fromQColor(cls, color, _canvas):
        result = cls()
        result.values = [color.redF(), color.greenF(), color.blueF(), color.alphaF()]
        return result
...
class _Action:
    ...
    def trigger(self):
        ...
        values = self.view.foregroundColor().components()
...
def test_canvas_bridge_preserves_asymmetric_components(host) -> None:
    document, view, target, line = host
    snapshot = adapter.snapshot_host(document, view, target, line, None, 9)
    bridge = adapter.canvas_color_bridge(view, snapshot.context)
    qcolor = bridge.source_rgb_to_qcolor((13, 117, 241))
    assert (qcolor.red(), qcolor.green(), qcolor.blue()) == (13, 117, 241)
    assert bridge.qcolor_to_source_rgb(qcolor) == (13, 117, 241)
